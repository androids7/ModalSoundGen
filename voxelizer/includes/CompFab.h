...
namespace CompFab
{
    ...
    typedef struct IndexVoxelGridStruct
    {
        //Square voxels only
        IndexVoxelGridStruct(Vec3 lowerLeft, unsigned int dimX, unsigned int dimY, unsigned int dimZ, precision_type spacing);
        ~IndexVoxelGridStruct();

        void save_binvox(const char * filename);

        inline int & isInside(unsigned int i, unsigned int j, unsigned int k)
        {
